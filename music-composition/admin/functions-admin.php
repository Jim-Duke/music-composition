...
# Registers composition details box sections, controls, and settings.
add_action( 'butterbean_register', 'mc_composition_details_register', 5, 2 );
...
function mc_admin_register_scripts() {
	...
	wp_register_script( 'mc-edit-composition', mc_plugin()->js_uri . "edit-composition{$min}.js", array( 'jquery', 'wp-util' ), '', true );
	...
	wp_localize_script( 'mc-edit-composition', 'mc_i18n', $i18n );
}
 ...
 * @return void
 */
function mc_composition_details_register( $butterbean, $post_type ) {

	if ( $post_type !== mc_get_composition_post_type() )
		return;

	$butterbean->register_manager( 'mc-composition',
		array(
			'post_type' => $post_type,
			'context'   => 'normal',
			'priority'  => 'high',
			'label'     => esc_html__( 'Composition Details:', 'music-composition' )
		)
	);

	$manager = $butterbean->get_manager( 'mc-composition' );
	...
	$url_args = array(
		...
		'description' => esc_html__( 'Enter the URL of the composition Web page.', 'music-composition' )
	);
	...
	$client_args = array(
		...
		'label'       => esc_html__( 'Client', 'music-composition' ),
		'description' => esc_html__( 'Enter the name of the client for the composition.', 'music-composition' )
	);
	...
	$location_args = array(
		...
		'description' => esc_html__( 'Enter the physical location of the composition.', 'music-composition' )
	);
	...
	$start_date_args = array(
		...
		'description' => esc_html__( 'Select the date the composition began.', 'music-composition' )
	);
	...
	$end_date_args = array(
		...
		'description' => esc_html__( 'Select the date the composition was completed.', 'music-composition' )
	);
	...
	$excerpt_args = array(
		...
		'section'     => 'description',
		'label'       => esc_html__( 'Description', 'music-composition' ),
		'description' => esc_html__( 'Write a short description (excerpt) of the composition.', 'music-composition' )
	);
...
function mc_get_settings_page_slug() {
	...
	return sprintf( '%s_page_mc-settings', mc_get_composition_post_type() );
}

/**
 * Returns an array of post formats allowed for the composition post type.
 *
 * @since  1.0.0
 * @access public
 * @return array
 */
function mc_get_allowed_composition_formats() {

	return apply_filters( 'mc_get_allowed_composition_formats', array( 'audio', 'gallery', 'image', 'video' ) );
}

/**
 * If a theme supports post formats, limit composition to only the audio, image,
...
function mc_post_format_support_filter() {
	...
	if ( current_theme_supports( 'post-formats' ) ) {
		...
		if ( isset( $formats[0] ) ) {
			$new_formats = array_intersect( $formats[0], mc_get_allowed_composition_formats() );
...
function mc_default_post_format_filter( $format ) {
	...
	return in_array( $format, mc_get_allowed_composition_formats() ) ? $format : 'standard';
}
